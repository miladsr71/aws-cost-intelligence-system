...
> **Status: deployed and running.** This pipeline has been executing daily at 8:00 AM Central on an EC2 instance (via cron) since April 2026 — pulling real billing data, checking for anomalies, and emailing alerts when something looks wrong.

## Dashboard Preview

![Dashboard](screenshots/dashboard.png)

*(Interactive HTML dashboard generated by the pipeline — cost trends, detected anomalies, and recommendation cards.)*
...
AWS CUR → S3 → Glue Crawler → Athena
                                  ↓
                          Python Pipeline (EC2, daily cron)
                                  ↓
              ┌─────────────────────────────────────┐
...
![SNS email alert](screenshots/alert-email.png)

*(Automated SNS email sent when an anomaly is detected — type, confidence, root cause, and recommended action.)*

## How to Run

### Prerequisites
- AWS account with CUR (Cost and Usage Reports) enabled
- Python 3.9+
...
The system is deployed to an EC2 instance (Amazon Linux) with a cron job for fully automated daily execution. No manual commands needed — the pipeline runs every morning, checks for anomalies, and sends alerts if anything unusual is found.
...
**Milad Zahmatkesh** — M.S. Computer Science, Southern Illinois University Edwardsville
AWS Certified Cloud Practitioner

- Portfolio: https://d1hc05jijwpwow.cloudfront.net
- LinkedIn: https://www.linkedin.com/in/miladza
- GitHub: https://github.com/miladsr71
